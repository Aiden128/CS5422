...
int boundedorbit(Iterate f, std::complex<T> seed, int bound, int bailout, 
...
int main(int argc, char **argv) {
    ...
    tbb::parallel_for(tbb::blocked_range2d<int>(0, height, 50, 0, width, 2), [&](auto r) {
        for (auto j = r.rows().begin(); j != r.rows().end(); j++) {
            for (auto k = r.cols().begin(); k != r.cols().end(); k++) {
                std::complex<double> c ((left + k*dx), (lower + j*dy));
                iteration_counts[width*j + k] = boundedorbit([&c](auto z) { return z*z + c; }, 0.0i, 2, iters, &normalized_iterations<double>);
            }
        }
    });
